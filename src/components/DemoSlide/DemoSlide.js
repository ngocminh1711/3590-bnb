...
function DemoSlide() {
  ...
  const handleOnNextClick = (index) => {
    const isLastSlide =
      currentIndex === topHouseForRent[index].image_view.length - 1;
  ...
  const handleOnPrevClick = (index) => {
    ...
    const newIndex = isFirstSlide
      ? topHouseForRent[index].image_view.length - 1
      : currentIndex - 1;
    setCurrentIndex(newIndex);
  };
          ...
          <div className="mx-auto max-w-2xl py-16 px-4 sm:py-10 sm:px-6 lg:max-w-7xl lg:px-8 ">
            ...
            <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
              {topHouseForRent.map((item, index) => (
                ...
                >
                  <div className=" aspect-w-1 aspect-h-1 w-full overflow-hidden rounded-2xl border bg-gray-200  ">
                    <div className=" w-full ">
                      ...
                      <div className=" w-full top-1/2 transform -translate-y-1/2 px-3 flex justify-between items-center">
                        <button
                          type="button"
                          onClick={() => handleOnPrevClick(index)}
                        >
                          <svg
                        ...
                        <button
                          type="button"
                          onClick={() => handleOnNextClick(index)}
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
