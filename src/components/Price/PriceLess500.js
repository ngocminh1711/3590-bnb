...
function PriceLess500() {
    ...
    const getHouseLess500 = async () => {
        return await axios.get('http://localhost:8000/api/products/less500')
    }
    ...
    useEffect(() => {
        ...
        getHouseLess500().then(res => {
            setHouseLess500(res.data.roomRates)
        })

    }, [])


    console.log(houseLess500)

    return (
        <div>
            {houseLess500.length > 0 ? (
